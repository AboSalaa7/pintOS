               ...
               thread_func *function, void *aux)
{
  ...
  if( thread_mlfqs ){
    ...
    real x1 = div_fixed_int(t->recent_cpu , 4 ) ;
    real x3 = int_to_fixed(64);
    x3 = sub_fixed_int(x3, t->nice * 2 );
    x3 = sub_two_fixed(x3,x1);
    priority =  fixed_to_nearest_int(x3);
    if( priority > 63 )
        priority = 63 ;
    else if ( priority < 0  )
        priority = 0 ;
...
/*
void thread_priority_donation( struct thread* target, int new_priority){

  target->old_priority = target->priority;
...
{
  thread_set_priority_or_donation(thread_current(),new_priority,false);
}

void
thread_set_priority_or_donation (struct thread *cur, int new_priority,
                           bool is_donated)
{
  enum intr_level old_level;
  old_level = intr_disable();
...
thread_set_nice (int nice UNUSED)
{
  ...
  real x1 = mul_fixed_int(load_avg,2);
  real x2 = add_fixed_int(x1,1);
  x1 = div_two_fixed(x1,x2);
  x1 = mul_two_fixed( x1 , temp->recent_cpu );
  temp->recent_cpu = add_fixed_int(x1 , temp->nice );
  x1 = div_fixed_int( temp->recent_cpu , 4  );
  x2 = int_to_fixed(64);
  x2 = sub_two_fixed(64,x1);
  x2 = sub_fixed_int(x2 , temp->nice * 2 );
  temp->priority = fixed_to_nearest_int(x2) ;
  if( temp->priority > 63 )
      temp->priority = 63 ;
...
{
  enum intr_level x = intr_disable();
  int w  = fixed_to_nearest_int(  mul_fixed_int(load_avg,100)  ) ;
  intr_set_level(x);
  return w ;
}
...
{
  enum intr_level x = intr_disable();
  int w = fixed_to_nearest_int( mul_fixed_int(thread_current()->recent_cpu , 100 ) )  ;
  intr_set_level(x);
  return w  ;
}
...
next_thread_to_run (void)
{
  if (list_empty (&ready_list))
    return idle_thread;
  if(thread_mlfqs){
    struct list_elem* e = list_max( &ready_list , comparator_greater_thread_priority , NULL );
    list_remove(e);
